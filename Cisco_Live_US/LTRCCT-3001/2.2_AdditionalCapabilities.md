...
- Click on new digital channels and Go to `Assets` > `Templates` and click the + icon besides Template Groups table header.
...
- You can search for transcripts using many fields depending on the channel. In this case use the Name field and search using the Customer Name used while testing the chat in previous lab. Once the conversation transcript shows up, click on the button in the last column.
...
- For example, click on resolve conversation node, you can see the HTTP request and response cycle on the right.
...
function nextLab() 
 {
 window.location.href = "https://ciscolivelabs.github.io/wxcclabguides/Cisco_Live_US/LTRCCT-3001/3.1_QnABotConfiguration.html";;
 }
function nextLabB() 
 {
